...
"""Create total compensation feature which reflects salary + bonus + total_stock_value"""
for key, value in data_dict.iteritems():
    if data_dict[key]['poi'] == False:
        if NaNCheck(data_dict[key]['salary']) or NaNCheck(data_dict[key]['bonus']) or \
                NaNCheck(data_dict[key]['total_stock_value']):
            data_dict[key]['compensation_ratio'] = 0
        else:
            data_dict[key]['compensation_ratio'] = float(data_dict[key]['total_stock_value']) / \
            ((float(data_dict[key]['salary']) + float(data_dict[key]['bonus']) +
              float(data_dict[key]['total_stock_value'])))
